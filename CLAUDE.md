...
**That fixture run skips 59 assertions, and they are the valuable ones.** Every
...
**main checkout**, against `~/code/cotdata_store`, the same suite is **494 passed / 5
skipped** rather than **435 / 64**.

> **From a worktree those two figures are 492 / 7 and 433 / 66**, because `test_boundaries`
...
```bash
bin/live-tests.sh          # the 59, against the real store. Scheduled 09:15 daily
```
