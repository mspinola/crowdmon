...
| [2026-08-03-index-share.md](2026-08-03-index-share.md) | **§1 EXECUTED 2026-08-03, genuine null. §2 blocked.** The only handoff here authored in another repo: written as cotdata#97 and moved here, because its analysis half must not be authored in the package whose boundary tests keep analysis out. Its `cotdata` blocker cleared with cotdata#96 and the producer action was run, adding `cot_supplemental` (13 markets, 1,074 weeks from 2006). **The premise is retired, exactly as §4 anticipated**: index positioning is not meaningfully stickier than swap. Swap is the MORE persistent at 12 weeks (median 0.826 vs 0.777, index ahead in only 4 of 13), index is 14% steadier week to week, and **under the worst 5% of weeks the swap book moves less than the index book** (-0.00167 vs -0.00336), adding to net long in 3 of 13 where index never does. Unsought finding: swap sits at **0.305** of Managed Money on routine turnover and **0.067** under stress, so `swap: 0.4` is incoherent **between regimes, not between markets**, the opposite of the per-market direction explored here. §0's cocoa premise is read off one week (swap holds the largest net long in 23.1% of 1,051 weeks; MM holds it 64.3%). **§2 remains blocked on §B33-B36, which do not exist** (the amendments file ends at B32), though §3's "22 of 39" is derivable from `2026-08-02 §B31` so the citation is stale rather than invented. **Metals are permanently outside coverage, so most of §0 is still open.** Weight table unchanged |
