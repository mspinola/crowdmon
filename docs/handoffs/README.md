...
| [2026-08-03-index-share.md](2026-08-03-index-share.md) | **BLOCKED, not started.** An empirical basis for the Swap Dealer `w = 0.4`. Blocked on **two** things, not the one its header names, both verified in its §5. (1) CIT Supplemental ingestion is **unwritten**, not merely unmerged: the `cotdata` handoff it depends on does not exist, `cotdata` has no `docs/handoffs/` at all, `get_cot` raises on anything but `legacy`/`disagg`/`tff`, and the Supplemental report is combined futures-and-options against a producer that ingests futures-only annual zips. (2) §2 cites **§B33-B36, which do not exist** — the amendments file ends at B32, and `A_agnostic` and the "22 of 39" figure appear nowhere. So the half that looked runnable against the present store is blocked **here**, not in `cotdata`. **No owner for either.** Order to unblock: `cotdata` Supplemental handoff, then §B33-B36, then this |
