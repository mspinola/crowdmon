...
| [2026-08-03-index-share.md](2026-08-03-index-share.md) | **COMPLETE as far as it can go, 2026-08-03.** The only handoff here authored in another repo (cotdata#97, moved here as PR #36; a second session filed a duplicate copy before checking the remote, which is what the claim-first convention below exists to stop). §1: **the premise is retired, exactly as §4 anticipated.** Index positioning is not meaningfully stickier than swap. Swap is the MORE persistent at 12 weeks (median 0.826 vs 0.777, index ahead in only 4 of 13), index is 14% steadier week to week, and **under the worst 5% of weeks the swap book moves less than the index book** (-0.00167 vs -0.00336), adding to net long in 3 of 13 where index never does. §2 was blocked on **§B33-B36, which did not exist**; they are now measured as **`2026-08-03 §C1-C4`** (filed under `C`, since amendments are one file per day and 08-02 closed at B32). The cited figures were **not invented**: "22 of 39" and "17 in both halves" reproduce exactly. Of §2's three headline figures one **cannot respond to `w_SD`** (the shape rule reads two nets and their signs), one (`A_agnostic`) is **undefined and degenerate at 1.0 on 100% of market-weeks**, and `Q_sell`/`Q_buy` answers: median `A` swings **42.0% on the 13 Supplemental markets** against 0.6% pooled, so **`w_SD` is load-bearing exactly where the handoff cares**. Unsought finding: swap sits at 0.305 of Managed Money on routine turnover and 0.067 under stress, so `swap: 0.4` is incoherent **between regimes, not between markets**. **Metals are permanently outside coverage, so most of §0 is still open.** Weight table unchanged |
| [2026-08-03-swap-dealer-weight-decision.md](2026-08-03-swap-dealer-weight-decision.md) | **COMPLETE 2026-08-03, decided by the human as option (a): `swap: 0.4` stands.** Filed as the follow-on the index-share handoff's §2 named. Its §4 released exactly one piece of work and that work **corrected the handoff's own §1**: `single_weight_sweep` shipped, and re-swept over the order-preserving band it showed the 42.0% headline was the wrong BAND rather than wrong arithmetic. 0.7 puts a swap dealer above `nonreportable` and `other_reportable`, which `2026-08-01 §A22` established is a different claim rather than a rival value. Honest figures: **17.9% on the 13 Supplemental markets, 3.5% pooled**, over `w_SD ∈ [0.2, 0.4]`. That is what made (a) defensible, and the decision was taken against those rather than against §1's. Also closed the floor: 0.1 exactly **ties** `producer_merchant` and collapses the distinction, which a stable sort hides, so the stress reading of 0.067 is unreachable without contradicting §6.3. **The regime incoherence is recorded, not resolved** (swap behaves at 0.067 under stress and carries 0.4), written into `core/config.py` beside the value so a kept weight cannot be mistaken for an unexamined one. Option (c), regime-conditional weights, is unscheduled rather than closed. Metals remain permanently outside coverage | 
