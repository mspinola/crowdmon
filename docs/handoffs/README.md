...
| [2026-08-03-index-share.md](2026-08-03-index-share.md) | **§1 EXECUTED 2026-08-03, genuine null. §2 still blocked.** An empirical basis for the Swap Dealer `w = 0.4`. Filed blocked on **two** things (§5); the first cleared when cotdata #96 merged and the producer action was run, adding `cot_supplemental` (13 markets, 1,074 weeks from 2006). **The premise is retired, exactly as §4 anticipated**: index positioning is not meaningfully stickier than swap. Swap is the MORE persistent at 12 weeks (median 0.826 vs 0.777, index ahead in only 4 of 13), index is 14% steadier week to week, and **under the worst 5% of weeks the swap book moves less than the index book** (-0.00167 vs -0.00336), adding to net long in 3 of 13 where index never does. Unsought finding worth carrying: swap sits at **0.305** of Managed Money on routine turnover and **0.067** under stress, so `swap: 0.4` is incoherent **between regimes, not between markets**, which is the opposite of the per-market direction this handoff explored. §0's cocoa premise is read off one week (swap holds the largest net long in 23.1% of 1,051 weeks; MM holds it 64.3%). **§2 remains blocked on §B33-B36, which do not exist** — the amendments file ends at B32. **Metals are permanently outside coverage, so most of §0 is still open.** Weight table unchanged. `2026-08-03-index-share.md` in `analysis/` |
