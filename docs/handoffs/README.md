...
| [2026-08-03-swap-dealer-weight-decision.md](2026-08-03-swap-dealer-weight-decision.md) | **OPEN, and deliberately so. It is a decision, not a measurement**, filed as the follow-on the index-share handoff's §2 named when it said "produce the evidence; the decision is separate and follows". The evidence is complete and is not the bottleneck: `swap: 0.4` is incoherent **between regimes** (0.305 of Managed Money on routine turnover, 0.067 under stress), and `w_SD` moves median `Q_sell/Q_buy` by **42.0% on the 13 Supplemental markets** against 0.6% pooled. Four options are stated and **none is recommended**, because choosing is the human's. A session may add the single-weight sweep `weight_sensitivity.py` still lacks; it may **not** edit `core/config.py`. Carries the three constraints that bound any answer: metals are permanently uncovered, template classification is too unstable to hang it on (`§C1`), and Producer/Merchant rather than swap is the load-bearing weight (`§A22`) |
