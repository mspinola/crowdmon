...
| [2026-08-03-index-share.md](2026-08-03-index-share.md) | **OPEN, unclaimed.** The first handoff here that is not closed, and the only one authored in another repo: written as cotdata#97 and moved here 2026-08-03, because its analysis half must not be authored in the package whose boundary tests keep analysis out. Its `cotdata` blocker cleared with cotdata#96, which landed the CIT Supplemental report. **Read its §3 before planning the work.** The binding constraint is coverage: 13 agricultural markets and **no metals**, so gold, silver and copper are outside it permanently, and gold is the market that motivated half the question (the swap dealer on the IMMOVABLE side). Index Traders is a Legacy-taxonomy carve-out and does NOT nest inside Disaggregated Swap Dealer, and the two reports are on different bases (combined vs futures-only), so no cross-report differencing |
