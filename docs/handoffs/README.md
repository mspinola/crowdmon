...
| [2026-08-03-b-series-recovery.md](2026-08-03-b-series-recovery.md) | **OPEN, filed 2026-08-03.** Its two drafted tasks were checked before filing and both premises had moved, which is recorded in its §0 so neither is re-opened: the inverted status marker is fixed and crowdmon#38 is closed, and **§B33-B37 were found rather than being absent**. They sit on `claude/template-followups-doc-corrections-45de1d` (`11b7c81`), **local-only, never pushed, no PR, 2,070 insertions**, so §1 is a data-loss recovery before anything else. That makes `2026-08-03 §C1-C4` a **blind re-derivation of existing work**: §C4 records `A_agnostic` as undefined where **§B34 defines it and measures a median of 3.0237**, and §B36 carries the cocoa `0.976 / 0.100` the index-share §3 cited verbatim. §2 reconciles the two lineages, §3 makes references path-plus-reproducer (104 bare `§B##` across 19 files), §4 runs `w_SD ∈ {0.067, 0.2, 0.4, 0.7}` under a **settled static-weights decision** and asks for the direction of bias. Weight table unchanged |
