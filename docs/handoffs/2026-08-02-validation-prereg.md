...
**Status:** open, jointly drafted, **sections 4 and 5 reserved for the commonality/impact session**
**Date:** 2026-08-02
**Target:** a session that has written none of this package, running through `crucible`
...
leave the package to be validated, **which is the seam**." Validation cannot live in crowdmon.
...
| where this document lives | commonality/impact session | reserved below |
...
2. **Where does this document live once complete?** It cannot be executed from crowdmon. The
   candidates are `npf` (which owns research governance) or `livebook` (which owns
   `PREREGISTRATION.md`), and neither session should choose unilaterally.
