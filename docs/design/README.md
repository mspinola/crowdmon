...
| [`amendments-2026-08-04.md`](amendments-2026-08-04.md) | **here** | D1 onward: what single number this package delivers. The raw `T` ranking is largely structural, two candidate replacement indices are degenerate by zero-sum, and `Phi` is neither inert nor monotone in `D`. Then the offside term, the pool-versus-signal defect it shipped with, and D13's composition of the two: the pool check removes half the CLOSE-and-SEVERE cell. **The open file** |
